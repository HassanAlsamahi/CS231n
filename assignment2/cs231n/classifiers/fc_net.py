...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores = relu_scores.dot(self.params['W2']) + self.params['b2']
        pass
        ...
        loss = np.sum(loss)
        loss /= num_train
        loss += self.reg * (np.sum(self.params['W1']*self.params['W1']) + np.sum(self.params['W2'] * self.params['W2'])) * 0.5
        pass

        #Gradients
        exp_sum = np.expand_dims(sum_exp,axis=1)
        softmax = exp_scores/exp_sum
        softmax[np.arange(num_train),y] -=1
        dscores = softmax

        dRelu = self.params['W2'].dot(softmax.T)
        dscores = (relu_scores > 0) * dRelu.T

        dw1 = X.T.dot(dscores)
        dw1 /= num_train
        dw1 += 2*self.reg*self.params['W1'] * 0.5

        dw2 = relu_scores.T.dot(softmax)
        dw2 /= num_train
